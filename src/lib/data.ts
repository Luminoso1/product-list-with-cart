...
export const dessertsData: Dessert[] = rowData.map((item) => ({
	...item,
	id: item.name,
	image: {
		thumbnail: getImageUrl(item.image.thumbnail),
		mobile: getImageUrl(item.image.mobile),
		tablet: getImageUrl(item.image.tablet),
		desktop: getImageUrl(item.image.desktop)
	}
}))
